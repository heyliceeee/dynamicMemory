...
void insertStudents(Students *students)
{
    ...
    {
        //if array is full, resize for add more students
        int newCapacity = 2 * MAX_STUDENTS;
        resizeStudents(students, newCapacity);
...
{
    (*students).count = 0;
    (*students).students = (Student*) malloc(initialCapacity*sizeof(Student));
}
...
{
    (*students).students = (Student*) realloc((*students).students, newCapacity * sizeof(Student));
}